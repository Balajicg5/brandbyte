...
export default function CampaignForm({
	...
	const loadAdCreatives = async () => {
		...
		try {
			const adCreativesData = await adCreativeService.getUserAdCreatives(
				user.$id,
			);
	...
	const handleMultiSelectChange = (
		field: "campaignGoals" | "targetPlatforms",
		value: string,
	) => {
		setFormData((prev) => ({
			...prev,
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			setSuccess(
				"Campaign saved successfully! You can now generate AI ad creatives.",
			);
		} catch (error: any) {
	...
	const generateAdCreative = async () => {
		...
		try {
			...
			setSuccess(
				"Background image generated successfully! Click 'Open Editor' to add text and finalize your poster.",
			);
			...
			setError(
				error.message || "Failed to generate ad creative. Please try again.",
			);
			setGenerationStep("");
			setGenerationProgress(0);
		} finally {
	...
	const openPosterEditor = () => {
		...
		router.push(
			`/dashboard/editor?campaign=${savedCampaign.$id}&image=${encodeURIComponent(imageUrl)}`,
		);
	};

	const downloadImage = async (
		imageUrl: string,
		filename: string = "ad-creative",
	) => {
		try {
	...
	const regenerateWithCustomPrompt = async () => {
		if (!editedPrompt.trim() || !selectedBrand || !user || !savedCampaign)
			return;
		...
		try {
			...
			setGenerationProgress(100);
			setSuccess(
				"Ad creative regenerated successfully with your custom prompt!",
			);
			setIsEditingPrompt(false);
			...
			setError(
				error.message || "Failed to regenerate ad creative. Please try again.",
			);
			setGenerationStep("");
			setGenerationProgress(0);
		} finally {
							...
							{adCreatives.length === 0 ? (
								<div className="text-center py-12">
									...
									<p className="text-gray-600 dark:text-gray-400">
										Create some campaigns and generate ad creatives to see them
										here!
									</p>
								</div>
							) : (
											...
											<div className="relative aspect-square overflow-hidden">
												...
												<div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
													<div className="flex space-x-2">
														<button
															onClick={() =>
																setViewingImage(adCreative.imageUrl)
															}
												...
												<div className="space-y-2 text-xs text-gray-500 dark:text-gray-400">
													<div className="flex items-center">
														...
														<span className="truncate">
															Campaign: {adCreative.campaignId || "Standalone"}
														</span>
													</div>
													<div className="flex items-center">
														<Target className="w-3 h-3 mr-1" />
														<span className="truncate">
															Brand: {adCreative.brandId}
														</span>
													</div>
													<div className="flex items-center">
							...
							<div>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
									{campaignGoalOptions.map((goal) => (
										<label
											key={goal}
											className="flex items-center space-x-2 cursor-pointer p-3 rounded-md border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
										>
											<input
												type="checkbox"
												checked={formData.campaignGoals.includes(goal)}
												onChange={() =>
													handleMultiSelectChange("campaignGoals", goal)
												}
									...
									{platformOptions.map((platform) => (
										...
										>
											<input
												...
												onChange={() =>
													handleMultiSelectChange("targetPlatforms", platform)
												}
												className="rounded border-gray-300 text-[#7A7FEE] focus:ring-[#7A7FEE]"
											/>
											<span className="text-sm text-gray-700 dark:text-gray-300">
								...
								{!generatedImage ? (
									<div className="text-center p-8 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
										...
										<p className="text-gray-600 dark:text-gray-400 mb-6">
											Your campaign details have been saved. Now let's create
											your AI-powered ad creative!
										</p>
										<button
											type="button"
											...
											<div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
												...
												<div className="text-center p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
													...
													<p className="text-gray-600 dark:text-gray-400 text-sm">
														Your background image is ready! Open the poster
														editor to add headlines, descriptions, and
														call-to-action text with professional design tools.
													</p>
												</div>
											</div>
										</div>
